import {Badge, Flex, Loader, Modal, Pagination} from '@mantine/core';
                ...
                {isLoading ?
                    ...
                    <>
                        <Flex direction={'row'} gap={'lg'} w={'100%'} wrap={'wrap'}>
                            {releases?.releases.map((release) => <Badge
                                onClick={() => navigate(`/release/${release.main_release}`)} className={classes.hover}
                                color={'violet'} size={'lg'}>{release.title}</Badge>)}
                        </Flex>
                        <Pagination mt={'lg'} total={releases?.pagination.pages} value={activePage} onChange={setPage}
                                    color={'violet'}/>
                    </>}
            </Modal>
        </>
